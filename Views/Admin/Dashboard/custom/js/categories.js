...
$(document).ready(function() {
	...
	$('#addCategoriesModalBtn').unbind('click').bind('click', function() {
		...
		$("#submitCategoriesForm").unbind('submit').bind('submit', function() {
			...
			if(categoriesName == "") {
				$("#categoriesName").after('<p class="text-danger" id = "vadility">Category Name field is required</p>');
				$('#categoriesName').closest('.form-group').addClass('has-error');
			} else {
				// remov error text field
				$("#categoriesName").find('#vadility').remove();
				// success out for form 
				$("#categoriesName").closest('.form-group').addClass('has-success');	  	
			}

			if(categoriesStatus == "") {
				$("#categoriesStatus").after('<p class="text-danger" id = "vadility2">Category Status field is required</p>');
				$('#categoriesStatus').closest('.form-group').addClass('has-error');
			} else {
				// remov error text field
				$("#categoriesStatus").find('#vadility2').remove();
				// success out for form 
	...
	if(categoriesId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				$("#editCategoriesStatus").val(response.categories_status);
				...
				$("#editCategoriesForm").unbind('submit').bind('submit', function() {
					...
					if(categoriesName == "") {
						$("#editCategoriesName").after('<p class="text-danger" id="vadility3">Category Name field is required</p>');
						$('#editCategoriesName').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#editCategoriesName").find('#vadility3').remove();
						// success out for form 
						$("#editCategoriesName").closest('.form-group').addClass('has-success');	  	
					}

					if(categoriesStatus == "") {
						$("#editCategoriesStatus").after('<p class="text-danger" id="vadility4">Category Status field is required</p>');
						$('#editCategoriesStatus').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#editCategoriesStatus").find('#vadility4').remove();
						// success out for form 
